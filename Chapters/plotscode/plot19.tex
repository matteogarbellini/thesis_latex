...
\begin{figure}[ht]
  ...
  \caption[]{\textbf{Probability distributions for the complete graph \bm{$C(51)$} with the time-dependent Hamiltonian.}The figure shows the probability distribution for a complete graph of $N=51$ using the time-dependent Hamiltonian with the Roland-Cerf $s_{RC}(t)$ (left) and linear $s_L(t)$ (right) interpolating schedules. It illustrates the great impact of the interpolating schedule on the overall performance of the time-dependent algorithm, where the choice of $s(t)$ is critical.}
